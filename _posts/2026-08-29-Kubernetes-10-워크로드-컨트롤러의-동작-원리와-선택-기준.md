...
`kubectl get`과 `describe`로 각 계층을 확인할 수 있다. 다음 명령으로 `controller-demo.yaml` 파일을 만든 뒤 적용하면 확인할 Deployment와 그 하위 ReplicaSet·Pod가 함께 만들어진다.

```bash
cat <<'EOF' > controller-demo.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
```bash
kubectl apply -f controller-demo.yaml
kubectl rollout status deployment/controller-demo
kubectl get deployment,replicaset,pod -l app=controller-demo
kubectl describe deployment controller-demo
kubectl describe replicaset "$(kubectl get replicaset -l app=controller-demo -o jsonpath='{.items[0].metadata.name}')"
```
